...
while not converged:
    ...
    if log_likelihood < best_log_likelihood + 1e-6:
        # Now, we want to say 'converge' if the current log likelihood becomes
        # lower than the best one (plus a certain error).
        converged = True

    if log_likelihood > best_log_likelihood:
        # Now, we want to update the best log likelihood if the current one is the the best one.
