...
'''
cost_fucntion = tf.multiply(tf.divide(1, 2), tf.reduce_mean(tf.pow(Y - hypothesis_function, 2)))

'''
Step 6: Using gradient descent with learning rate of 0.001 to minimize loss
'''
optimizer = tf.train.GradientDescentOptimizer(learning_rate=0.001).minimize(cost_fucntion)
...
with tf.Session() as session:
    ...
    for i in range(30000):
        ...
        cost = session.run(cost_fucntion, feed_dict={X: data.T[0], Y: data.T[1]})
    ...
    training_cost = session.run(cost_fucntion, feed_dict={X: data.T[0], Y: data.T[1]})
